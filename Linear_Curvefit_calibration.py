...
# = 'V'.
x_name = 'Scale reading'
x_units = 'g'
y_name = 'Weight from hung masses'
y_units = 'N'
...
plt.xlabel('{} [{}]'.format(x_name,x_units), fontsize = 14)
plt.ylabel('{} [{}]'.format(y_name,y_units), fontsize = 14)
plt.title('Linear fit of Applied weight vs. \n Scale reading for calibration', fontsize = 16)
...
plt.errorbar(x,residual,yerr=y_sigma,marker='.',linestyle='',
             label="Residual (Weight Data - Fit)")
plt.hlines(0,np.min(x),np.max(x),lw=2,alpha=0.8)
plt.xlabel('{} [{}]'.format(x_name,x_units), fontsize = 14)
plt.ylabel('Weight Data - Fit [{}]'.format(y_units), fontsize = 14)
plt.legend(loc=4,numpoints=1, fontsize = 12)
plt.title('Residual plot of linear fit', fontsize = 16)
plt.tick_params(axis='both', which='major', labelsize=10)
plt.show()
plt.bar(bins[:-1],hist,width=bins[1]-bins[0])

plt.ylim(0,1.2*np.max(hist))
plt.xlabel('(y-y_fit)/y_sigma')
...
plt.vlines(-1.0,0,np.max(hist)*1.3,lw=3,color='r',linestyles='--')
plt.vlines(+1.0,0,np.max(hist)*1.3,lw=3,color='r',linestyles='--',label='+/- error')
plt.text(0.0,np.max(hist)*1.1,'{:.1f}% of data within one sigma of fit'.format(within_err),
